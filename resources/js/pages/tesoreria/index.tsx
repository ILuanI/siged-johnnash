...
import { index as tesoreriaIndex, show as tesoreriaShow } from '@/actions/App/Http/Controllers/Tesoreria/EstadoCuentaController';
                ...
                {alumnos.data.length === 0 ? (
                    ...
                    <ul className="space-y-2">
                        {alumnos.data.map((estudiante: any) => {
                            ...
                            const cuotas = lastMatricula?.comprobante_pago?.cuotas || [];
                                ...
                                <li key={estudiante.id_alumno}>
                                    <div className="flex w-full items-center gap-4 rounded-xl border bg-white p-4 text-left">
                                        ...
                                        <div className="min-w-0 flex-1">
                                            ...
                                            <p className="text-sm text-slate-500">
                                                {estudiante.dni ? `DNI ${estudiante.dni}` : 'Sin DNI'}
                                            </p>
                                        </div>
                                        <div className="flex items-center gap-4">
                                            <div className="flex flex-col items-end gap-1">
                                                <Badge className={cn('shrink-0 rounded-full text-xs uppercase', estadoBadgeClass(estudiante.estado))}>
                                                    {estudiante.estado}
                                                </Badge>
                                                {cuotas.length > 0 ? (
                                                    <SemaforoPagos cuotas={cuotas} />
                                                ) : (
                                                    <span className="text-xs text-slate-400">Sin plan de pago</span>
                                                )}
                                            </div>
                                            <Button asChild variant="outline" size="sm">
                                                <Link href={tesoreriaShow.url({ alumno: estudiante.id_alumno })}>
                                                    Ver Cuenta
                                                </Link>
                                            </Button>
                                        </div>
